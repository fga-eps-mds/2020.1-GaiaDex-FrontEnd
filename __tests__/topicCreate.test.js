...
const wrapper = mount(<TopicCreate navigation={{ getParam: jest.fn() }}/>);

describe('<TopicCreate />', () => {
  it('renders everything', () => {
    const tree = renderer
      .create(<TopicCreate navigation={{ getParam: jest.fn() }}></TopicCreate>)
      .toJSON();
  ...
  it('renders View correctly', () => {
    expect(wrapper.find('View').at(0)).toHaveStyle(styles.TopicCreatemasterView);
    ...
    expect(wrapper.find('View').at(10)).toHaveStyle(styles.TopicCreatetituloDiv);
    expect(wrapper.find('View').at(14)).toHaveStyle(styles.TopicCreatescrollDiv);

  });
  it('renders Text correctly', () => {
    expect(wrapper.find('Text').at(2)).toHaveStyle({ color: 'white', fontSize: 15 });
    expect(wrapper.find('Text').at(6)).toHaveStyle(styles.TopicCreatepublicarText);
  ...
  it('renders TouchableOpacity correctly', () => {
    expect(wrapper.find('TouchableOpacity').at(0)).toHaveStyle(styles.TopicCreatepublicarDiv);
  ...
  it('renders Text Input correctly', () => {
    expect(wrapper.find('TextInput').at(0)).toHaveStyle(styles.TopicCreatetituloTopic);
    expect(wrapper.find('TextInput').at(1)).toHaveStyle(styles.TopicCreatetopicComment);
    expect(wrapper.find('TextInput').at(0)).toHaveProp('placeholder', 'Titulo do post...');
    expect(wrapper.find('TextInput').at(1)).toHaveProp('placeholder', 'Conteudo do post...');
